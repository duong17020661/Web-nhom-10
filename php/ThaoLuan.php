...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(isset($_POST['search'])){
        ...
        $result = mysqli_query($conn, "SELECT count(id_question) as total FROM question WHERE question like '%$timkiem%' AND id_topic = $id");
    }
}
...
$danhsach = mysqli_query($conn, "SELECT * FROM question WHERE id_topic = $id LIMIT $start, $limit");
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(isset($_POST['search'])){
        $timkiem = $_POST['timkiem'];
        $danhsach = mysqli_query($conn, "SELECT * FROM question WHERE question like '%$timkiem%' AND id_topic = $id LIMIT $start, $limit");
    }
}
...
<div>
    ...
    if($danhsach->num_rows >0){
        while ($row = mysqli_fetch_assoc($danhsach)){
            $remove = $row['id_question']."rm";
            $edit = $row['id_question']."edit";
            $sum_answer = $row['id_question'];
            $socautraloi = mysqli_query($conn, "SELECT COUNT(*) as sum FROM answer WHERE id_question = $sum_answer");
            $sum = mysqli_fetch_assoc($socautraloi);
            ...
            echo"&emsp;Số câu trả lời: ".$sum['sum']."";
            echo"</p>";
            echo"</div>";
            ?>
            <script>
