...
$query_id_pin = "SELECT id_check FROM question WHERE id_question = $id_question";
$result_id_pin = mysqli_query($conn,$query_id_pin);
if ($result_id_pin->num_rows > 0) {
    // output data of each row
    while($row = $result_id_pin->fetch_assoc()) {
        $id_pin =  $row['id_check'];
    }
}
...
<body>
    <div class="container">
        ...
        <div class="shadow p-3 mb-5 bg-white rounded" style="margin-bottom:70px;">
            ...
            <div class="card" id="question">
                ...
                $id = mysqli_real_escape_string($conn, $_GET['ques']);
                ...
                <div class="card-body" id="question-body">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            if($row['id_answer'] == $id_pin){
                                echo "Câu trả lời đã ghim";
                            ...
                            echo "" . $row['answer'] . "";
                            echo "</p>";
                            echo '<a href=pin.php?id=' . $id . '&id_pin=' . $row['id_answer'].'&del=1>'.'<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-remove"></span></button></a>';
                            echo "</div>";
                            echo "</div>";
                        break;
                            }
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "" . $row['answer'] . "";
                            echo "</p>";
                            echo '<a href=pin.php?id=' . $id . '&id_pin=' . $row['id_answer'] . '&del=0><button type="button" class="btn btn-default btn-sm"
                                                ><span class="glyphicon glyphicon-paperclip"></span></button></a>';
                            echo "</div>";
                            echo "</div>";
                        }
                    }
                    ...
                    <div class="form-group">
                        ...
                        <form method='post'>
                            <?php
                            if($_SESSION['login'] != ""){
                            echo"<label for='exampleFormControlTextarea1'>Trả lời</label>";
                            echo"<textarea class='form-control' id='exampleFormControlTextarea1' rows='3' name='answer'></textarea>";
                               echo" <br>";
                            echo"<input class='btn btn-primary' type='submit' value='Trả lời' style='width:100px' name='create'>";
                            }
                            ?>
    ...
    <div class="text-center">
        <!-- Footer -->
        <a href="Trangchu.html#menu" style="border-radius: 75%;position:fixed;right:10%"><img src="images/btt.png" height="40px" width="auto" /></a>
